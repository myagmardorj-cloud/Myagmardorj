...
\midrule
\texttt{zeros\_ht} & $\approx 74{,}920$ & 100,000 & Low height \\
\texttt{zeros1} & $\sim 10^{12}$     & $\sim$2M & High-T block \\
\texttt{zeros6} & $\sim 10^{13}$     & $\sim$10K & High-T block \\
...
\texttt{zeros\_ht} & $\approx 74{,}920$ & 0.5113 & bootstrap UNSTABLE \\
...
The ratio $R(p) = A(p)/B(p)$ varies across datasets; no stable constant is observed. No theoretical derivation exists.
...
\item \textbf{Scale dependence.} $r$ varies across datasets: 0.68 ($N=100$K), 0.51 ($N=10$K), 0.45 ($N=2$M). No convergence to $r = 0.6847 (zeros1)$ observed. Bootstrap stability improves with $N$ but full-dataset $r$ does not.
\item \textbf{No theoretical derivation.} The BK amplitude constant $C$ has no analytic explanation.
\item \textbf{Independent replication failed.} An attempt to reproduce $r = 0.6847 (zeros1)$ on the Odlyzko $10^{12}$ zero block (\texttt{zeros\_ht.txt}, $N=10{,}000$, $T \approx 2.68 \times 10^{11}$) yielded $r = 0.51$, not $r = 0.6847 (zeros1)$. The exact dataset and normalization parameters that produced the original result have not been fully documented. This is a critical unresolved issue.
\end{enumerate}
...
\item Theoretical derivation of the BK amplitude constant $C$ from explicit formula
...
cd prime-locked-zeros && python analyze.py ../zeros1
# Expected: r = 0.6847 (zeros1), naive p = 2.64e-15 (uncorrected)
\end{verbatim}
