...
```bash
wget http://www-users.cse.umn.edu/~odlyzko/zeta_tables/zeros3 -O zeros1.txt
```
...
## Expected results (zeros1)

| | Expected |
|-|----------|
| Real data r | 0.45–0.68 (dataset-dependent) |
| Shuffled null mean r | ~0.0 +/- 0.2 |
| GUE surrogate mean r | ~0.0 +/- 0.2 |

## Troubleshooting

If r differs significantly from the range 0.45–0.68:
1. Normalization: must be log(p)/log(T/2pi), NOT log(p)/2pi
2. Data file: use zeros1.txt (high-T block, T ~ 10^12)
