...
platform_pages = [
    ...
    "verify.html", "roadmap.html", "contact.html"
]

paper_pages = list((ROOT / "paper1").glob("*.html"))

COMMON_CSS = """
/* === MASTER FIXED NAV / FOOTER === */
...
document.addEventListener('click', function(e){
  if(!e.target.closest('.MN-more')){
    document.querySelectorAll('.MN-dd').forEach(function(d){
...
def fix_page(path, nav, footer):
    ...
    html = re.sub(r"<nav[\s\S]*?</nav>", nav, html, count=1, flags=re.I)
    html = re.sub(r"<footer[\s\S]*?</footer>", footer, html, count=1, flags=re.I)

    # if no nav/footer
    if "<nav class=\"MN\">" not in html:
        html = html.replace("<body class=\"en\">", "<body class=\"en\">\n" + nav, 1)
    if "<footer class=\"MN-foot\">" not in html:
        html = html.replace("</body>", footer + "\n</body>", 1)

    # inject CSS
    if "MASTER FIXED NAV / FOOTER" not in html:
        html = html.replace("</style>", COMMON_CSS + "\n</style>", 1)

    # replace old duplicated language scripts by adding master script before body
    if "function MN_toggle(btn)" not in html:
        html = html.replace("</body>", COMMON_JS + "\n</body>", 1)
...
for name in platform_pages:
    p = ROOT / name
    if p.exists():
        fix_page(p, PLATFORM_NAV, PLATFORM_FOOTER)

for p in paper_pages:
    fix_page(p, PAPER_NAV, PAPER_FOOTER)

print("DONE: menus normalized")