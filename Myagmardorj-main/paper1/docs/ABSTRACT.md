...
**Preprint:** https://zenodo.org/records/20077673

---

We report a numerical observation: consistency of a prime-correlated
excess signal across tested prime ranges in the spacing covariance
statistics of high Riemann zero blocks.

Using Odlyzko zero data at heights T ~ 10^12 and 10^13, we compute
the spacing covariance C(h) at prime lags h = p and compare with
the Bogomolny-Keating (BK) predictor B(p) = (log p)^2 / p.
Empirical agreement with BK-type amplitude scaling is observed
after high-T normalization (tau_p = log(p)/log(T/2pi)).

Pearson r ranges from 0.4509 to 0.6847 across tested datasets.
