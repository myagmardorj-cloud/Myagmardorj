...
\midrule
\texttt{zeros2} & $\approx 74{,}920$ & 0.9783 & $3.82\times10^{-186}$ \\
\texttt{zeros3} & $\sim 10^{12}$     & 0.9992 & $2.64\times10^{-15}$  \\
\texttt{zeros4} & $\sim 10^{13}$     & 0.9421 & $5.0\times10^{-6}$    \\
\bottomrule
\end{tabular}
...
\item \textbf{Unknown asymptotics.} Behavior of $r$ as $N \to \infty$ unknown; tested only up to $N=10{,}000$.
\item \textbf{No theoretical derivation.} $C \approx 16.5$ has no analytic explanation.
\item \textbf{No independent replication.} Results not yet reproduced on a separate machine/codebase.
\end{enumerate}
...
The signal passes several null controls across three independent datasets.
